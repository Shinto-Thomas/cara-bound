# Example Client: Complete CARA Trial Workflow
# This script demonstrates how to use the CARA API for a clinical trial
...
cat("========================================\n")
cat("CARA API - Example Trial Workflow\n")
cat("========================================\n\n")
...
init_response <- api_call("POST", "/trial/initialize", body = list(
  study_name = "Hypertension-CARA-Example",
  n0 = 10,
  gamma = 2,
  target = "Neyman",
  TB = 1,
...
if (!is.null(export)) {
  # Save to file
  write_json(export, "trial_data_export.json", pretty = TRUE)
  cat("Data exported to: trial_data_export.json\n")
}
